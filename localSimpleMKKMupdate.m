function [Sigma,Hstar,CostNew] = localSimpleMKKMupdate(KH,Sigma,GradNew,NN,CostNew,numclass,option,MM,lambda)
...
%  Projected gradient
%-----------------------------------------------------
%[~,Hstar] = localCostSimpleMKKM(KH,stepmax,desc,NN,SigmaNew,numclass,MM,lambda);
while costmax<costmin
    [costmax,Hstar] = localCostSimpleMKKM(KH,stepmax,desc,NN,SigmaNew,numclass,MM,lambda);
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax)) && stepmax > eps
    ...
    [costmedr,Hstarr] = localCostSimpleMKKM(KH,stepmedr,desc,NN,SigmaNew,numclass,MM,lambda);
    [costmedl,Hstarl] = localCostSimpleMKKM(KH,stepmedl,desc,NN,SigmaNew,numclass,MM,lambda);     
